...
public class MultiAction {
	...
	public ModelAndView hello4(Date date) {
		...
		mv.addObject("handlerMethod", date.toString());
		mv.setViewName("multiactiontest");
		return mv;
	}
//test url http://localhost:8080/springmvc/hello5?personName=12345&user=winzip;password;1000
//test customer property editor
	@RequestMapping("/hello5")
	public ModelAndView hello5(Person p) {
		ModelAndView mv = new ModelAndView();
		System.out.println(p.getPersonName());
		mv.addObject("handlerMethod", "hello5");
		mv.setViewName("multiactiontest");
		return mv;
	}
	
	// 注释掉这个 InitBinder 那么 conversion-service 将起作用
	@InitBinder
